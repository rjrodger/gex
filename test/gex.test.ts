...
describe('Gex', function () {
  ...
  test('arrays', () => {
    ...
    assert.equal(
      s(a_.on(['ab', null, 'dd', undefined, 'ee', NaN])),
      s(['ab'])
    )
  })
  ...
  test('dodgy', () => {
    assert.equal(Gex(null as any).on('aaa'), null)
    assert.equal(Gex(NaN as any).on('aaa'), null)
    assert.equal(Gex(undefined as any).on('aaa'), null)
  ...
  test('escapes', () => {
    ...
    assert.equal((g.re as any)().toString(), '/^a\\*b$/')
    assert.equal(g.on('a*b'), 'a*b')
    assert.equal(g.on('a**b'), null)
  ...
  test('newlines', () => {
    var g = Gex('a*b')
    assert.equal('/^a[\\s\\S]*b$/', '' + (g.re as any)())
    assert.equal(g.on('a\nb'), 'a\nb')
  })
  ...
  test('zero', () => {
    ...
    assert.equal(Gex(1 as any).on('1'), '1')
    assert.equal(Gex(100 as any).on('100'), '100')
    assert.equal(Gex(0 as any).on('0'), '0')
  })
  ...
  test('multi', () => {
    ...
    assert.equal(s((g.re as any)()), '{"a":{},"b":{}}')
  ...
  test('funky', () => {
    ...
    assert.equal(Gex(null as any).on('a'), null)
    assert.equal(Gex(undefined as any).on('a'), null)
    assert.equal(Gex(NaN as any).on('a'), null)
    assert.equal(Gex(/a/ as any).on('a'), null)
  })
})
